...
export default function Home() {
  return (
    <main className="flex-grow pt-24 pb-32 relative z-10 overflow-x-hidden transition-colors">
      ...
      <footer className="relative overflow-hidden bg-[#0B0D14] mt-20 border-t-4 border-orange-500">
        {/* Cinematic Indian Democracy Background */}
        <div className="absolute inset-0 z-0">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-[#0B0D14] via-transparent to-[#0B0D14]"></div>
        </div>

        {/* Top Government Banner */}
        <div className="relative z-10 w-full bg-orange-500/10 border-b border-orange-500/20 backdrop-blur-md">
          <div className="max-w-7xl mx-auto px-8 py-3 flex flex-wrap items-center justify-between text-[10px] sm:text-xs font-bold text-orange-200/80 uppercase tracking-widest">
            <div className="flex items-center gap-4">
               <span className="material-symbols-outlined text-sm text-orange-500">account_balance</span>
               A National Initiative for Electoral Integrity
            </div>
            <div className="flex items-center gap-4 mt-2 sm:mt-0">
               <a href="#" className="hover:text-white transition-colors">GOI Web Directory</a>
               <span className="opacity-40">|</span>
               <a href="#" className="hover:text-white transition-colors">National Portal of India</a>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-8 py-20 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-16 items-start">
            <div className="md:col-span-5 space-y-8">
              ...
              >
                 <div className="w-16 h-16 bg-gradient-to-br from-orange-500 to-orange-700 rounded-xl flex items-center justify-center shadow-[0_0_30px_rgba(249,115,22,0.3)] border border-orange-500/40">
                    <span className="material-symbols-outlined text-white text-4xl font-black">how_to_vote</span>
                 </div>
                 <div>
                   <h1 className="text-4xl font-black tracking-tighter text-white font-headline">VoteSaathi</h1>
                   <p className="text-xs font-bold text-orange-400 tracking-[0.2em] uppercase mt-1">Digital Consulate of India</p>
                 </div>
              </motion.div>
              <p className="text-sm text-white/60 font-medium leading-relaxed max-w-sm">
                Empowering 1.4 Billion citizens with sovereign AI intelligence, verifiable integrity, and localized civic education. 
                Celebrating the world's largest democratic festival uniting every faith, language, and culture.
              </p>
                 ...
                 <div className="flex gap-3">
                    {['facebook', 'twitter', 'linkedin', 'youtube'].map((s, i) => (
                      <motion.div 
                         key={s} 
                         whileHover={{ y: -3 }}
                         className={`w-10 h-10 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-white/60 hover:bg-orange-500 hover:text-white hover:border-orange-500 transition-all cursor-pointer`}
                      >
            ...
            <div className="md:col-span-7 grid grid-cols-2 sm:grid-cols-3 gap-10">
               <div className="space-y-6">
                  <h4 className="text-[11px] font-black uppercase tracking-[0.25em] text-white/40 border-b border-white/10 pb-4">Government</h4>
                  <ul className="space-y-3 text-sm font-semibold text-white/70">
                     {['Election Commission', 'Ministry of Law', 'National Voter Portal', 'Voter Helpline'].map(link => (
                       <li key={link}>
               ...
               <div className="space-y-6">
                  <h4 className="text-[11px] font-black uppercase tracking-[0.25em] text-white/40 border-b border-white/10 pb-4">Resources</h4>
                  <ul className="space-y-3 text-sm font-semibold text-white/70">
                     {['Electoral Roll', 'Know Your Candidate', 'Booth Locator', 'Result Analytics'].map(link => (
                       <li key={link}>
                         <Link href="#" className="hover:text-orange-400 transition-colors flex items-center gap-2 group">
                           <span className="w-1.5 h-1.5 rounded-full bg-orange-500/50 group-hover:bg-orange-400 transition-colors"></span>
                           {link}
               ...
               <div className="space-y-6 col-span-2 sm:col-span-1">
                  <h4 className="text-[11px] font-black uppercase tracking-[0.25em] text-white/40 border-b border-white/10 pb-4">Information</h4>
                  <ul className="space-y-3 text-sm font-semibold text-white/70">
                     {['Privacy Policy', 'Terms of Service', 'Copyright Policy', 'Hyperlinking Policy', 'Accessibility Statement'].map(link => (
                       <li key={link}>
                         <Link href="#" className="hover:text-orange-400 transition-colors flex items-center gap-2 group">
                           <span className="w-1.5 h-1.5 rounded-full bg-orange-500/50 group-hover:bg-orange-400 transition-colors"></span>
                           {link}
            ...
            <div className="flex items-center gap-4">
              <div className="text-center md:text-left space-y-1">
                <p className="text-[10px] font-bold uppercase tracking-[0.1em] text-white/40">
                  © 2024 VoteSaathi Intelligence Mesh.
                </p>
                <p className="text-[10px] font-semibold text-white/30">
                  Content managed by the Department of Civic Technologies. Designed and Developed by NIC.
                </p>
              </div>
            </div>
            
            <div className="flex gap-6 items-center">
              <div className="flex gap-4 text-[9px] font-black uppercase tracking-widest text-orange-500/80 bg-orange-500/10 px-4 py-2 rounded-lg border border-orange-500/20">
                 ...
                 <span className="opacity-40">|</span>
                 <span className="text-white/60">V1.4.2-BHARAT</span>
              </div>
            </div>
          </div>
        </div>
