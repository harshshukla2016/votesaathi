        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              href={item.href}
              className={`flex flex-col items-center justify-center min-w-[4rem] px-2 py-1 transition-all ${
                isActive ? "text-orange-500" : "text-on-surface-variant hover:text-on-background"
              }`}
            >
              <span className={`material-symbols-outlined text-2xl ${isActive ? "fill" : ""}`}>
                {item.icon}
              </span>
              <span className={`text-[9px] font-bold mt-0.5 whitespace-nowrap ${isActive ? "opacity-100" : "opacity-70"}`}>
                {item.name}
              </span>
            </Link>
          );
